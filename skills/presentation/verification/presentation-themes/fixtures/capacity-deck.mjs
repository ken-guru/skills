import { renderPresentationMarkdown } from '../../../generate-slides/scripts/semantic-markup.mjs';

export function capacityDeck(
  themeId,
  manifest,
  {
    portraitFilename = 'media/portrait.svg',
    portraitAlt = 'A protected portrait fixture representing a collaborator shaping an idea',
  } = {},
) {
  const slides = [
    { role: 'opener', title: ['Ideas take', 'shape', 'together'], label: 'Collaborative intelligence', subtitle: ['Human judgment gives direction.', 'AI gives momentum.'], visual: { type: 'picture', filename: portraitFilename, alt: portraitAlt }, capacity: { titleLines: 3, subtitleLines: 2 } },
    ...
    { heading: ['Make the reasoning', 'visible'], body: [['Externalize incomplete', 'ideas early.'], ['Keep the central subject', 'clearly protected.'], ['Use feedback to sharpen', 'the shared model.'], ['Preserve intent while', 'changing presentation.']], caption: ['Portrait media selects the portrait composition', 'without changing the slide’s semantic role.'], visual: { type: 'picture', filename: portraitFilename, alt: portraitAlt, intendedOrientation: 'portrait', actualOrientation: 'portrait' }, capacity: { headingLines: 2, bullets: 4, bulletLines: 2, captionLines: 2 } },
    { heading: ['One table, several', 'useful perspectives'], body: [], caption: [], visual: { type: 'picture', filename: 'media/collaboration-landscape.png', alt: 'Two collaborators shaping a physical model together at a wide studio worktable', intendedOrientation: 'full-image', actualOrientation: 'full-image' }, capacity: { headingLines: 2, bullets: 0, bulletLines: 0, captionLines: 0 } },
