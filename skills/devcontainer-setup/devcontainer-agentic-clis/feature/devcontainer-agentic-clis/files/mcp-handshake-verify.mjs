#!/usr/bin/env node
// Verifies an MCP server by completing a real `initialize` handshake over
// stdio, rather than just checking the binary exists. Called by
// staged-install-handshake-atomic-swap.sh's health_check_root -- not
// meant to be run standalone.
//
// Usage: mcp-handshake-verify.mjs <command> [args...]
...
if (!command) {
	console.error('Usage: mcp-handshake-verify.mjs <command> [args...]');
	process.exit(2);
}
