...
- **Already exists**: this repo already has a Shared Container from a prior run. Nothing to
  ...
  Exception: if `devcontainer.json` has no `containerEnv.BASH_ENV` entry, it predates the fix that
  makes `GH_TOKEN` (and the rest of `.devcontainer/.env`) visible to an AI CLI's own tool calls —
  without it, `GH_TOKEN` only ever reaches the three lifecycle scripts (postCreate/postStart/
  postAttach), never a `gh` command an agent runs itself, even right after a token was added and
  the container rebuilt. Patch it in place: add `.devcontainer/bash-env.sh` (copy of
  [templates/bash-env.sh](templates/bash-env.sh), chmod +x), add
  `"containerEnv": {"BASH_ENV": "/workspace/.devcontainer/bash-env.sh"}` to `devcontainer.json`,
...
- `.devcontainer/Dockerfile` ← [templates/Dockerfile](templates/Dockerfile), copied verbatim (no
  placeholders).
- `.devcontainer/code-runner.sh` ← [templates/code-runner.sh](templates/code-runner.sh), copied
  verbatim, chmod +x. The Dockerfile's `COPY code-runner.sh
  /usr/local/bin/devcontainer-code-runner` depends on this file existing in the
  build context.
...
- `.devcontainer/devcontainer.json` ← [templates/devcontainer.json](templates/devcontainer.json)
  ...
  later append to. `containerEnv.BASH_ENV` points at `bash-env.sh` above — this is what makes
  `GH_TOKEN` visible to an AI CLI's own tool calls, not just the lifecycle scripts.
- `.devcontainer/post-create.sh` — generated by
  [scripts/render-devcontainer.sh](scripts/render-devcontainer.sh) (`--help` documents exactly
  what it assembles and in what order):
...
## 6. Host-side walkthrough

The remaining steps happen on the host, outside the Shared Container — do
the safe, verifiable ones yourself and report the result; ask before
touching the user's shell profile; hand off SSH key material entirely
rather than generating or touching it yourself.

- **Docker + Dev Containers extension.**

  ```bash
  docker info >/dev/null 2>&1 && echo "docker: ok" || echo "docker: not available"
  code --list-extensions 2>/dev/null | grep -qx ms-vscode-remote.remote-containers \
    && echo "extension: ok" || echo "extension: not installed"
  ```

  If either check fails, print the install links and stop the walkthrough
  here until the user confirms they're ready — installing Docker Desktop or
  launching it is out of scope for this skill:
  - Docker Desktop: <https://www.docker.com/products/docker-desktop/>
  - Dev Containers extension: `ms-vscode-remote.remote-containers`

- **`GH_TOKEN`.** Print, don't fetch: a repository-scoped **fine-grained**
  PAT is required — <https://github.com/settings/personal-access-tokens/new>
  (not the classic-token page). State the scopes: Issues and pull requests
  read/write, repository contents read, workflow files write, workflow
  status/history read, Dependabot/advisories/code scanning/secret
  scanning/security events read. No Administration, secrets/variables
  management, or workflow-run mutation.

If step 3's answer was yes, also walk through:

- **Credential Directory.** Propose
  `~/.devcontainer-credentials/{{REPO_SLUG}}` (or `{{REPO_NAME}}` for a
  Local Checkout, since `{{REPO_SLUG}}` is empty there) and ask the user to
  confirm or override it. Once confirmed, create it yourself:

  ```bash
  mkdir -p "$CREDENTIAL_DIR" && chmod 700 "$CREDENTIAL_DIR"
  ```

- **Deliver `GH_TOKEN` / `DEVCONTAINER_CREDENTIALS_DIR` into the host
  shell.**

  ```bash
  command -v direnv >/dev/null 2>&1 && echo "direnv: available" || echo "direnv: not found"
  ```

  - **Available**: offer to write `.envrc` at the repo root (sibling to
    `.git/`, not inside `.devcontainer/` — direnv triggers on directory
    entry, not container attach) from
    [templates/envrc.example](templates/envrc.example), substituting
    `{{GH_TOKEN}}` and, if step 3 applies, `{{CREDENTIAL_DIR}}` (drop that
    line entirely otherwise), then `direnv allow`. Add `.envrc` to
    `.gitignore` in the same step, since it can carry `GH_TOKEN`. Only
    write it if the user accepts.
  - **Not available**: mention <https://direnv.net> as an optional
    suggestion, but don't offer to install it — that's a bigger, more
    persistent footprint than this walkthrough should take on unprompted.
  - Either way, print the export lines for the user's own shell profile
    and ask before appending anything to it yourself — only append
    idempotently, with a clear marker comment (matching the pattern this
    skill already uses inside the container's own `~/.bashrc`), if they
    explicitly accept.

- **SSH deploy/signing keys.** Hand this off entirely. Copy
  [templates/provision-ssh-keys.sh](templates/provision-ssh-keys.sh)
  somewhere the user can run it, `chmod +x` it, and tell them to run it —
  optionally with `CREDENTIAL_DIR` and `DEPLOY_KEY_LABEL` already exported
  (the Credential Directory from above, and `{{REPO_SLUG}}` respectively)
  so it doesn't re-ask for values already confirmed. It checks for a
  reusable signing key across repos, always generates a fresh repo-scoped
  deploy key, places both in the Credential Directory, and prints both
  public keys. GitHub registration itself happens later, via
  `post-attach.sh`'s prompt once the container is open — this script never
  calls the GitHub API or registers anything.

Finally, regardless of SSH:

- Reopen the repo in the container (**Dev Containers: Reopen in
  Container**).
- Run whichever `setup-<tool>-devcontainer` skill(s) you want, to add AI
  CLIs.
