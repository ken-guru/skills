...
while [ "$#" -gt 0 ]; do
  case "$1" in
    --agent) AGENT="$2"; shift 2;; --target) TARGET="$2"; shift 2;; --source) SOURCES+=("$2"); shift 2;; --skill) SKILLS+=("$2"); shift 2;; -h|--help) usage; exit 0;; *) usage; exit 64;;
  esac
done
[ -n "$AGENT" ] && [ -n "$TARGET" ] && [ "${#SOURCES[@]}" -gt 0 ] || { usage; exit 64; }
CONFIG_DIR="$(dirname "$TARGET")"; CURRENT_MANIFEST="$CONFIG_DIR/.skills-manifest"; PREVIOUS_MANIFEST="$CONFIG_DIR/.skills-manifest.previous"; mkdir -p "$CONFIG_DIR"
STAGE="$(mktemp -d "$CONFIG_DIR/.skills-refresh.XXXXXX")"; STAGE_HOME="$STAGE/home"; STAGE_TARGET="$STAGE_HOME/${TARGET#"$HOME"/}"
mkdir -p "$STAGE_TARGET"; cleanup(){ rm -rf "$STAGE"; }; trap cleanup EXIT
for source in "${SOURCES[@]}"; do
  selected=(--skill '*'); if [ "${#SKILLS[@]}" -gt 0 ]; then selected=(); for skill in "${SKILLS[@]}"; do selected+=(--skill "$skill"); done; fi
...
find "$STAGE_TARGET" -name SKILL.md -type f -print -quit | grep -q . || { echo "Skill refresh produced no valid skills" >&2; exit 1; }
