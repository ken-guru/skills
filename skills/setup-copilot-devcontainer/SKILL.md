...
opted into), `.env.example`, and `README.md`, and idempotently ensures its own Network
...
Tell the user: rebuild or reopen the container. For auth, create a
fine-grained PAT at https://github.com/settings/personal-access-tokens/new
whose resource owner is their **personal account**, with only the
**Copilot Requests** account permission, and add it to `.devcontainer/.env` as
`COPILOT_GITHUB_TOKEN` (the commented line in `.env.example` shows it). No
...
authenticates from `GH_TOKEN` on its own — that's outdated (this skill never rewrites a block it already
wrote), and the `.env.example` block is correct. If they previously answered
"yes" to Copilot's "Store token in plain text config file?" prompt, they
should run `/logout` in `copilot` to remove that stored token;
`COPILOT_GITHUB_TOKEN` replaces it.
